...
internal fun DictionaryWordDTO.toDictionaryWord(): DictionaryWord {
    val mappedElements = dictionaryElement.map { dtoElement ->
        when (dtoElement) {
            is DictionaryElementDTO.PartSpeech -> {
                DictionaryElement.PartSpeech(dtoElement.partSpeech)
            }
            is DictionaryElementDTO.Synonyms -> {
                DictionaryElement.Synonyms(dtoElement.dictionarySynonymsDTO.toDictionarySynonyms())
            }
        }
    }
    ...
    return DictionaryWord(
        ...
        dictionaryElement = ArrayList(mappedElements)
    )
}

internal fun DictionarySynonymsDTO.toDictionarySynonyms(): DictionarySynonyms {
    return DictionarySynonyms(
        id = id,
        word = word,
        translation = translation
    )
